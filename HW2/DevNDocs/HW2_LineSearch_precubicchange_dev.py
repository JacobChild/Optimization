...
def pinpoint(pttf, phitf, phiptf, methodf, mu1f, mu2f, pf,  solve_func = sphere_func, maxiter = 100): #pt_tracker, phi_tracker, phiprime_tracker
    ...
    while k <= maxiter:
        a1f = alphafunc(pttf[-2], pttf[0],pf) 
        a2f = alphafunc(pttf[-1], pttf[0],pf)
        ...
        if methodf == 'quad': # for quad I don't yet know phiprime2 and phi(pt2) > phi(pt1) 
            ...
            a_high = pttf[-1]
            phi_high = pttf[-1]
            ap = (2*a1f * np.abs(phitf[-1,0] - phitf[-2,0]) + phiptf[-1]*(a1f**2 - a2f**2)) / (2 * np.abs(phitf[-1,0] - phitf[-2,0] + phiptf[-1](a1f-a2f)))
        ...
        if methodf == 'cubic': # I should know phiprime2, and phi(pt2) < phi(pt1)
           ...
           if phitf[-1, 0] < phitf[-2, 0]:
            ...
            #! I think this is off below and make phiprime 2 correlate with low and high from diagram.
           beta1 =  phiptf[-2,0] + phiptf[-1,0] - 3* (phitf[-2,0] - phitf[-1,0]) / (a1f - a2f)
           beta2 = np.sign(a2f - a1f) * np.sqrt(beta1**2 - phiptf[-2,0]*phiptf[-1,0])
           ap = a2f - (a2f - a1f) * (phiptf[-1,0] + beta2 -beta1) / (phiptf[-1,0] - phiptf[-2,0] + 2* beta2)
